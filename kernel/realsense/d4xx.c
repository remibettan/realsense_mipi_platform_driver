...
static int ds5_raw_write( struct ds5 * state, u16 reg, const void * val, size_t val_len )
{
    ...
    if( ret < 0 )
        ...
        dev_dbg( &state->client->dev, "%s(): i2c raw write 0x%04x: %d bytes\n", __func__, reg, (int)val_len );
...
static const char * ds5_get_sensor_name( struct ds5 * state )
{
    ...
    int sensor_id = state->is_rgb * 1 + state->is_depth * 2 + state->is_y8 * 3 + state->is_imu * 4;
                                                         ...
                                                         const struct ds5_resolution ** best )
{
    ...
    for( i = 0, res = fmt->resolutions; i < fmt->n_resolutions; i++, res++ )
    {
        unsigned long delta = abs( ffmt->width * ffmt->height - res->width * res->height );
        if( delta < best_delta )
        {
            best_delta = delta;
            *best = res;
...
static int __ds5_sensor_set_fmt( struct ds5 * state,
                                 struct ds5_sensor * sensor,
#if LINUX_VERSION_CODE < KERNEL_VERSION( 5, 15, 10 )
                                 struct v4l2_subdev_pad_config * cfg,
#else
                                 ...
                                 struct v4l2_subdev_format * fmt )
{
    ...
    sensor->config.format = ds5_sensor_find_format( sensor, mf, &sensor->config.resolution );
    ...
    dev_dbg( sensor->sd.dev,
             "%s(): fmt->pad: %d, sensor->mux_pad: %d, code: 0x%x, %ux%u substream:%d\n",
             __func__,
             fmt->pad,
             sensor->mux_pad,
             fmt->format.code,
...
#ifdef CONFIG_VIDEO_D4XX_SERDES
static int ds5_setup_pipeline( struct ds5 * state, u8 data_type1, u8 data_type2, int pipe_id, u32 vc_id )
{
    int ret = 0;
    dev_warn( &state->client->dev,
			 ...
			 data_type2: 0x%x, vc_id: %u\n",
              ...
              data_type2,
              vc_id );
    ret |= max9295_set_pipe( state->ser_dev, pipe_id, data_type1, data_type2, vc_id );
    ret |= max9296_set_pipe( state->dser_dev, pipe_id, data_type1, data_type2, vc_id );
    if( ret )
        dev_warn( &state->client->dev,
...
static int ds5_configure( struct ds5 * state )
{
    ...
    ret = ds5_setup_pipeline( state, data_type1, data_type2, sensor->pipe_id, vc_id );
                  ...
                  || sensor->config.format->data_type == GMSL_CSI_DT_CUSTOM_IR_RGB_16 ) )
    {
        ...
        else
        {
            dev_err( sensor->sd.dev,
                     "%s(): Illegal mbus_code %u for IR sensor\n",
                     __func__,
...
static int ds5_sensor_g_frame_interval( struct v4l2_subdev * sd, struct v4l2_subdev_frame_interval * fi )
{
    struct ds5_sensor * sensor = container_of( sd, struct ds5_sensor, sd );
    ...
    dev_dbg( sd->dev, "%s(): %s %u\n", __func__, sd->name, fi->interval.denominator );

    return 0;
}
static u16 __ds5_probe_framerate( const struct ds5_resolution * res, u16 target );

static int ds5_sensor_s_frame_interval( struct v4l2_subdev * sd, struct v4l2_subdev_frame_interval * fi )
{
    struct ds5_sensor * sensor = container_of( sd, struct ds5_sensor, sd );
    u16 framerate = 1;
...
{
    if( val != V4L2_EXPOSURE_APERTURE_PRIORITY && val != V4L2_EXPOSURE_MANUAL )
        return -EINVAL;
...
static int ds5_hw_set_exposure( struct ds5 * state, u32 base, s32 val )
{
    ...
    if( ! ret )
        ret = ds5_write( state, base | DS5_MANUAL_EXPOSURE_LSB, (u16)( val & 0xffff ) );
...
static int ds5_get_hwmc( struct ds5 * state, unsigned char * data, u16 cmdDataLen, u16 * dataLen )
{
    int ret = 0;
    u16 tmp_len = 0;
    ...
    if( ret )
    {
        dev_dbg( &state->client->dev, "%s(): HWMC status not clear, ret: %d\n", __func__, ret );
    ...
    ret = ds5_raw_read( state, DS5_HWMC_RESP_LEN, &tmp_len, sizeof( tmp_len ) ); /* Read response length */
    if( ret )
        return -EBADMSG;
    ...
    dev_dbg( &state->client->dev, "%s(): HWMC read len: %d, lrs_len: %d\n", __func__, tmp_len, tmp_len - 4 );
...
{
    struct ds5 * state = container_of( ctrl->handler, struct ds5, ctrls.handler );
    ...
    case DS5_CAMERA_CID_LASER_POWER:
        if( ! state->is_rgb )
            ret = ds5_write( state, base | DS5_LASER_POWER, ctrl->val );
        break;
    case DS5_CAMERA_CID_MANUAL_LASER_POWER:
        if( ! state->is_rgb )
            ret = ds5_write( state, base | DS5_MANUAL_LASER_POWER, ctrl->val );
        break;
    case DS5_CAMERA_DEPTH_CALIBRATION_TABLE_SET:
        dev_dbg( &state->client->dev, "%s(): DS5_CAMERA_DEPTH_CALIBRATION_TABLE_SET \n", __func__ );
        if( ctrl->p_new.p )
        {
            struct hwm_cmd * calib_cmd;
            dev_dbg( &state->client->dev, "%s(): table id: 0x%x\n", __func__, *( (u8 *)ctrl->p_new.p + 2 ) );
            if( DEPTH_CALIBRATION_ID == *( (u8 *)ctrl->p_new.p + 2 ) )
            {
                calib_cmd = devm_kzalloc( &state->client->dev, sizeof( struct hwm_cmd ) + 256, GFP_KERNEL );
                if( ! calib_cmd )
                {
                    dev_err( &state->client->dev, "%s(): Can't allocate memory for 0x%x\n", __func__, ctrl->id );
                    ret = -ENOMEM;
                    break;
                }
    ...
    case DS5_CAMERA_COEFF_CALIBRATION_TABLE_SET:
        dev_dbg( &state->client->dev, "%s(): DS5_CAMERA_COEFF_CALIBRATION_TABLE_SET \n", __func__ );
        if( ctrl->p_new.p )
        {
            struct hwm_cmd * calib_cmd;
            dev_dbg( &state->client->dev, "%s(): table id %d\n", __func__, *( (u8 *)ctrl->p_new.p + 2 ) );
    ...
    case DS5_CAMERA_CID_AE_SETPOINT_SET:
        if( ctrl->p_new.p_s32 )
        {
            ...
            ret = ds5_send_hwmc( state, sizeof( struct hwm_cmd ) + 4, ae_setpoint_cmd );
            if( ! ret )
                ret = ds5_get_hwmc_status( state );
    ...
    case DS5_CAMERA_CID_HWMC_RW:
        if( ctrl->p_new.p_u8 )
        {
            ...
            size |= *( (u8 *)ctrl->p_new.p_u8 + 0 );
            ret = ds5_send_hwmc( state, size + 4, (struct hwm_cmd *)ctrl->p_new.p_u8 );
        }
        break;
    case DS5_CAMERA_CID_SYNC_MODE:
        ...
        if( state->is_depth )
        {
            ...
            dev_info( &state->client->dev,
                      "%s(): SYNC_MODE command passed to FW, addr: 0x%x, value: %d, ret: %d\n",
                      __func__,
    ...
    case V4L2_CID_IPU_SET_SUB_STREAM: {
        ...
        dev_info( &state->client->dev,
                  "V4L2_CID_IPU_SET_SUB_STREAM %x vc_id:%d, substream:%d, on:%d\n",
                  val,
                  vc_id,
                  substream,
                  on );
...
static int ds5_get_calibration_data( struct ds5 * state, enum table_id id, unsigned char * table, unsigned int length )
{
    ...
    ret = ds5_raw_read( state, DS5_HWMC_RESP_LEN, &table_length, sizeof( table_length ) ); /* Read response length */
...
static int ds5_g_volatile_ctrl( struct v4l2_ctrl * ctrl )
{
    struct ds5 * state = container_of( ctrl->handler, struct ds5, ctrls.handler );
    u16 log_prepare[]
        = { 0x0014, 0xcdab, 0x000f, 0x0000, 0x0400, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000 };
    ...
    case DS5_CAMERA_CID_LOG:
        ...
        ret = ds5_raw_write( state,
                             DS5_HWMC_DATA, /* Write command data */
                             log_prepare,
                             sizeof( log_prepare ) );
        if( ret < 0 )
            return ret;

        ret = ds5_raw_write( state, DS5_HWMC_EXEC, &execute_cmd, sizeof( execute_cmd ) ); /* execute cmd */
        if( ret < 0 )
            return ret;

        for( i = 0; i < DS5_MAX_LOG_POLL; i++ )
        {
            ret = ds5_raw_read( state, DS5_HWMC_STATUS, &data, sizeof( data ) );
...
static int ds5_board_setup( struct ds5 * state )
{
    ...
    state->g_ctx.dst_csi_port = ( ! strcmp( str_value, "a" ) ) ? GMSL_CSI_PORT_A : GMSL_CSI_PORT_B;
    ...
    state->g_ctx.src_csi_port = ( ! strcmp( str_value, "a" ) ) ? GMSL_CSI_PORT_A : GMSL_CSI_PORT_B;
    ...
    state->g_ctx.serdes_csi_link = ( ! strcmp( str_value, "a" ) ) ? GMSL_SERDES_CSI_LINK_A : GMSL_SERDES_CSI_LINK_B;
...
static int ds5_board_setup( struct ds5 * state )
{
    ...
    for( i = 0; i < MAX_DEV_NUM; i++ )
    {
        ...
        {
            dev_info( dev,
                      "MAX9296 found device on %d@0x%x\n",
                      serdes_inited[i]->dser_i2c->adapter->nr,
    ...
    if( state->aggregated )
        suffix += 4;
    dev_info( dev,
              "Init SerDes %c on %d@0x%x<->%d@0x%x\n",
              suffix,
              bus,
    ...
    dev_info( dev,
              "Address reassignment for %s-%c 0x%x->0x%x\n",
...
MODULE_PARM_DESC( serdes_bus,
                  "max9295/6 deserializer i2c bus\n"
                  "\t\tserdes_bus=2,2,4,4" );
...
MODULE_PARM_DESC( des_addr,
                  "max9296 deserializer i2c address\n"
...
static int ds5_ctrl_init( struct ds5 * state, int sid )
{
    ...
    if( sid == DEPTH_SID || sid == IR_SID )
    {
        ctrls->gain = v4l2_ctrl_new_std( hdl, ops, V4L2_CID_ANALOGUE_GAIN, 16, 248, 1, 16 );
    }
    else if( sid == RGB_SID )
    {
        ctrls->gain = v4l2_ctrl_new_std( hdl, ops, V4L2_CID_ANALOGUE_GAIN, 0, 128, 1, 64 );
    }
    ...
    if( sid >= DEPTH_SID && sid < IMU_SID )
    {
        ...
        ctrls->auto_exp
            = v4l2_ctrl_new_std_menu( hdl,
                                      ...
                                      ~( ( 1 << V4L2_EXPOSURE_MANUAL ) | ( 1 << V4L2_EXPOSURE_APERTURE_PRIORITY ) ),
                                      V4L2_EXPOSURE_APERTURE_PRIORITY );
    ...
    if( sid == DEPTH_SID || sid == IR_SID )
    {
        ctrls->exposure = v4l2_ctrl_new_std( hdl, ops, V4L2_CID_EXPOSURE_ABSOLUTE, 1, MAX_DEPTH_EXP, 1, DEF_DEPTH_EXP );
    }
    else if( sid == RGB_SID )
    {
        ctrls->exposure = v4l2_ctrl_new_std( hdl, ops, V4L2_CID_EXPOSURE_ABSOLUTE, 1, MAX_RGB_EXP, 1, DEF_RGB_EXP );
    }
    ...
    if( ctrls->query_sub_stream )
        ctrls->query_sub_stream->flags |= V4L2_CTRL_FLAG_VOLATILE | V4L2_CTRL_FLAG_EXECUTE_ON_WRITE;
    ...
    default:
        ...
        dev_dbg( state->mux.sd.subdev.dev, "%s():%d set ctrl_handler for MUX\n", __func__, __LINE__ );
        break;
    }
...
static int ds5_sensor_init( struct i2c_client * c,
                            struct ds5 * state,
                            struct ds5_sensor * sensor,
...
#else
    snprintf( sd->name, sizeof( sd->name ), "D4XX %s %d-%04x", name, i2c_adapter_id( c->adapter ), c->addr );
#endif
...
static int ds5_sensor_register( struct ds5 * state, struct ds5_sensor * sensor )
{
    ...
    ret = media_create_pad_link( entity,
                                 0,
                                 &state->mux.sd.subdev.entity,
                                 sensor->mux_pad,
...
static int ds5_ir_init( struct i2c_client * c, struct ds5 * state )
{
    ...
    return ds5_sensor_init( c, state, &state->ir.sensor, &ds5_ir_subdev_ops, "ir" );
}
                                   ...
                                   struct v4l2_subdev_mbus_code_enum * mce )
{
    ...
    case DS5_MUX_PAD_EXTERNAL:
        if( mce->index >= state->ir.sensor.n_formats + state->depth.sensor.n_formats )
            return -EINVAL;
                            ...
                            struct v4l2_subdev_format * fmt )
{
    ...
    dev_dbg( sd->dev,
             "%s(): fmt->pad:%d, sensor->mux_pad:%u size:%d-%d, code:0x%x field:%d, color:%d\n",
             __func__,
             fmt->pad,
             pad,
             fmt->format.width,
...
/* Video ops */
static int ds5_mux_g_frame_interval( struct v4l2_subdev * sd, struct v4l2_subdev_frame_interval * fi )
{
    struct ds5 * state = container_of( sd, struct ds5, mux.sd.subdev );
    struct ds5_sensor * sensor = NULL;
...
static int ds5_mux_s_frame_interval( struct v4l2_subdev * sd, struct v4l2_subdev_frame_interval * fi )
{
    struct ds5 * state = container_of( sd, struct ds5, mux.sd.subdev );
    struct ds5_sensor * sensor = NULL;
    u16 framerate = 1;
...
static int ds5_mux_s_stream( struct v4l2_subdev * sd, int on )
{
...
#endif
    dev_dbg( &state->client->dev,
             "s_stream for stream %s, vc:%d, SENSOR=%s on = %d\n",
             sensor->sd.name,
             vc_id,
             ds5_get_sensor_name( state ),
             on );
    ...
    if( on )
    {
...
#endif
        sensor->pipe_id = max9296_get_available_pipe_id( state->dser_dev, (int)state->g_ctx.dst_vc );
        if( sensor->pipe_id < 0 )
        {
        ...
        ret = ds5_write( state, DS5_START_STOP_STREAM, DS5_STREAM_START | stream_id );
        if( ret < 0 )
            goto restore_s_state;
        ...
        {
            dev_err( &state->client->dev, "start streaming failed, exit on timeout\n" );
            /* notify fw */
    ...
    {  // off
        ...
        if( state->is_y8 && state->ir.sensor.config.format->data_type == GMSL_CSI_DT_RGB_888 )
        {
            max9296_reset_oneshot( state->dser_dev );
        }
...
static int ds5_mux_get_frame_desc( struct v4l2_subdev * sd, unsigned int pad, struct v4l2_mbus_frame_desc * desc )
{
    unsigned int i;
...
static int ds5_hw_init( struct i2c_client * c, struct ds5 * state )
{
    ...
    if( ! ret )
        dev_dbg( sd->dev,
                 "%s(): %d: %u lanes, phy %x, data rate %u-%u\n",
                 __func__,
                 __LINE__,
                 n_lanes,
                 phy,
...
#ifdef CONFIG_TEGRA_CAMERA_PLATFORM
    dev_dbg( sd->dev,
             "%s(): %d phandle %x node %s status %x\n",
             __func__,
             __LINE__,
             c->dev.of_node->phandle,
             c->dev.of_node->full_name,
             mipi_status );
#endif
...
static int ds5_mux_init( struct i2c_client * c, struct ds5 * state )
{
    ...
    v4l2_set_subdevdata( sd, state );
#ifdef CONFIG_OF
    snprintf( sd->name, sizeof( sd->name ), "DS5 mux %d-%04x", i2c_adapter_id( c->adapter ), c->addr );
#else
    if( state->aggregated )
        suffix += 4;
    ...
    if( state->is_depth )
    {
        ...
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_depth, NULL );
#else
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_depth, NULL, true );
#endif
        state->mux.last_set = &state->depth.sensor;
    }
    else if( state->is_rgb )
    {
#if LINUX_VERSION_CODE < KERNEL_VERSION( 4, 20, 0 )
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_rgb, NULL );
#else
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_rgb, NULL, true );
#endif
        state->mux.last_set = &state->rgb.sensor;
    }
    else if( state->is_y8 )
    {
#if LINUX_VERSION_CODE < KERNEL_VERSION( 4, 20, 0 )
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_y8, NULL );
#else
        v4l2_ctrl_add_handler( &state->ctrls.handler, &state->ctrls.handler_y8, NULL, true );
#endif
        state->mux.last_set = &state->ir.sensor;
    }
    else
...
static int ds5_dfu_switch_to_dfu( struct ds5 * state )
{
    ...
    ds5_raw_write_with_check( state,
                              DS5_HWMC_DATA,
                              &cmd_switch_to_dfu,
                              sizeof( cmd_switch_to_dfu ) ); /* Write command data */
    ds5_write_with_check( state, DS5_HWMC_EXEC, 0x01 );      /* execute cmd */
...
static int ds5_dfu_wait_for_get_dfu_status( struct ds5 * state, enum dfu_fw_state exp_state )
{
    ...
    do
    {
        ...
        do
        {
            ...
            else if( status == 0x0002 && dfu_wr_wait_msec )
        ...
        dfu_wr_wait_msec
            = ( ( (unsigned int)dfu_asw_buf[3] ) << 16 ) | ( ( (unsigned int)dfu_asw_buf[2] ) << 8 ) | dfu_asw_buf[1];
    }
...
#ifdef CONFIG_TEGRA_CAMERA_PLATFORM
    struct i2c_adapter * parent = i2c_parent_is_i2c_adapter( state->client->adapter );
#endif
    mutex_lock( &state->lock );
...
static int ds5_dfu_device_release( struct inode * inode, struct file * file )
{
    ...
    dev_dbg( &state->client->dev,
             "%s(): i2c-%d bus_clk %d, restore to %d\n",
             __func__,
             i2c_adapter_id( parent ),
             i2c_get_adapter_bus_clk_rate( parent ),
    ...
    if( ret )
    {
        dev_warn( &state->client->dev, "%s(): no communication with d4xx\n", __func__ );
        mutex_unlock( &state->lock );
        return ret;
    }
    ret = ds5_read( state, DS5_FW_BUILD, &state->fw_build );
...
static const struct file_operations ds5_device_file_ops = { .owner = THIS_MODULE,
...
static int ds5_chrdev_init( struct i2c_client * c, struct ds5 * state )
{
    ...
    *dev_num = MKDEV( MAJOR( *dev_num ), MINOR( *dev_num ) );
    ...
    if( state->aggregated )
        suffix += 4;
    snprintf( dev_name, sizeof( dev_name ), "%s-%c", DS5_DRIVER_NAME_DFU, suffix );
#else
    snprintf( dev_name, sizeof( dev_name ), "%s-%d-%04x", DS5_DRIVER_NAME_DFU, i2c_adapter_id( c->adapter ), c->addr );
#endif
...
static void ds5_substream_init( struct ds5 * state )
{
    ...
    set_sub_stream_dt( state->pad_to_substream[DS5_MUX_PAD_RGB], mbus_code_to_mipi( MEDIA_BUS_FMT_YUYV8_1X16 ) );
...
/* SYSFS attributes */
#ifdef CONFIG_SYSFS
static ssize_t ds5_fw_ver_show( struct device * dev, struct device_attribute * attr, char * buf )
{
    ...
    return snprintf( buf,
                     PAGE_SIZE,
                     "D4XX Sensor: %s, Version: %d.%d.%d.%d\n",
                     ds5_get_sensor_name( state ),
                     ( state->fw_version >> 8 ) & 0xff,
                     state->fw_version & 0xff,
                     ( state->fw_build >> 8 ) & 0xff,
                     state->fw_build & 0xff );
}
 ...
 */
static ssize_t ds5_read_reg_show( struct device * dev, struct device_attribute * attr, char * buf )
{
    u16 rbuf;
    int n;
    struct i2c_client * c = to_i2c_client( dev );
    struct ds5 * state = container_of( i2c_get_clientdata( c ), struct ds5, mux.sd.subdev );
    struct dev_ds5_reg_attribute * ds5_rw_attr = container_of( attr, struct dev_ds5_reg_attribute, attr );
 ...
 */
static ssize_t ds5_read_reg_store( struct device * dev, struct device_attribute * attr, const char * buf, size_t count )
{
    struct dev_ds5_reg_attribute * ds5_rw_attr = container_of( attr, struct dev_ds5_reg_attribute, attr );
    int rc = -1;
    u32 reg;
...
#define DS5_RW_REG_ATTR( _name )                                                                                       \
    struct dev_ds5_reg_attribute dev_attr_##_name                                                                      \
        = { __ATTR( _name, S_IRUGO | S_IWUSR, ds5_read_reg_show, ds5_read_reg_store ), 0, 0 }

static DS5_RW_REG_ATTR( ds5_read_reg );

static ssize_t
ds5_write_reg_store( struct device * dev, struct device_attribute * attr, const char * buf, size_t count )
{
    struct i2c_client * c = to_i2c_client( dev );
    struct ds5 * state = container_of( i2c_get_clientdata( c ), struct ds5, mux.sd.subdev );
...
static struct attribute * ds5_attributes[]
    = { &dev_attr_ds5_fw_ver.attr, &dev_attr_ds5_read_reg.attr.attr, &dev_attr_ds5_write_reg.attr, NULL };
...
static int ds5_probe( struct i2c_client * c, const struct i2c_device_id * id )
{
    ...
    if( ret < 0 )
    {
        dev_err( &c->dev, "%s(): cannot communicate with D4XX: %d on addr: 0x%x\n", __func__, ret, c->addr );
        goto e_regulator;
    }
    ...
    dev_info( &c->dev,
              "D4XX Sensor: %s, firmware build: %d.%d.%d.%d\n",
              ds5_get_sensor_name( state ),
              ( state->fw_version >> 8 ) & 0xff,
              state->fw_version & 0xff,
              ( state->fw_build >> 8 ) & 0xff,
...
static int ds5_remove( struct i2c_client * c )
{
    ...
    if( state->dfu_dev.dfu_state_flag != DS5_DFU_RECOVERY && state->mux.sd.subdev.v4l2_dev )
    {
#ifdef CONFIG_SYSFS
...
static struct i2c_driver ds5_i2c_driver = {
    .driver = { .owner = THIS_MODULE, .name = DS5_DRIVER_NAME },
    .probe = ds5_probe,
    .remove = ds5_remove,
    .id_table = ds5_id,
};
